 ...
 *
 * 이 서비스가 내보내는 이벤트는 pet.profile.updated 하나입니다.
 * 그래서 목록에 뜨는 것도 그 토픽뿐이며, 멈춰 있으면 verdict 가 옛 판정을 계속 쓰게 됩니다.
 */
@Slf4j
@Service
...
public class AdminOutboxService {
     ...
     * 멈춘 건이 같은 반려동물의 뒤 이벤트를 막지 않기 때문에,
    ...
    public void republish(UUID outboxId) {
        ...
        if (!published) {
            ...
            throw new CustomException(PetErrorCode.OUTBOX_REPUBLISH_FAILED);
        }
