...
public class S3StorageProvider implements StorageProvider {
    ...
    public Optional<String> extractOwnedKey(String url, UUID accountId) {
        ...
        // 우리 버킷의 주소로 시작하는지 봄
        String prefix = PUBLIC_URL_FORMAT.formatted(
                storageProperties.bucket(), storageProperties.region(), "");
        ...
        //
        // 접두사만 보면 pets/{내accountId}/../{남의자리} 같은 값이 지나갈 수 있으므로
        // 경로를 거슬러 올라가는 표기가 섞였는지도 함께 거름
        if (!key.startsWith(OWNED_PREFIX_FORMAT.formatted(accountId)) || key.contains("..")) {
            return Optional.empty();
        }
